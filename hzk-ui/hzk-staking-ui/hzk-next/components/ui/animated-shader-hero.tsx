...
void main(){gl_Position=position;}`;
    ...
    render(now = 0) {
      ...
      gl.uniform2fv((program as any).pointers, new Float32Array(this.pointerCoords as number[]));
  ...
  class PointerHandler {
    ...
    constructor(element: HTMLCanvasElement, scale: number) {
      ...
      element.addEventListener('pointerup', (e) => {
        if (this.count === 1) {
          this.lastCoords = this.first as number[];
        }
    ...
    get first() {
      return (this.pointers.values().next().value as number[] | undefined) || this.lastCoords;
    }
  }
  ...
  const loop = (now: number) => {
    ...
    rendererRef.current.updateMouse(pointersRef.current.first as number[]);
    rendererRef.current.updatePointerCount(pointersRef.current.count);
    rendererRef.current.updatePointerCoords(pointersRef.current.coords as number[]);
    rendererRef.current.updateMove(pointersRef.current.move as number[]);
